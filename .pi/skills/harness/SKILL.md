...
11. For setup/capability questions, do not make the human remember internal paths or environment variables. Prefer `/harness`, `/harness-models`, `/harness-local-llm`, `/harness-team`, `/harness-research`, `/harness-memory`, `/harness-brief`, or shell commands like `ph more`, `ph models`, `ph team`, and `ph research`.
12. If the task is vague, use the `/harness-brief` style: ask a few targeted questions, identify risk, create/fill a task packet, and propose verification before edits.
13. For task-scoped MCP/subagent access, use `harness_policy_profile` and keep profiles narrow/expiring (`mcp-discovery`, `subagent-review`, or exact `mcp-direct-selected` tools). Prefer `clearOnFinish: true` and validate connector classifications with `npm run tool:policy -- metadata --json`.
...
17. Prefer the done flow (`harness_done_task`, `/harness-done`, or `ph done`) when a task is ready; it runs project checks, auto-plans risk-based review, writes a proof ledger, drafts/validates evidence, and runs finish gates.
18. If the done flow is unavailable, finish manually with `harness_write_evidence` and `harness_finish_task`, then fix any task, evidence, external-write, memory, review, secret-scan, provenance, policy, lock, or eval findings before saying the work is done.
